...
grid_resolution = 0.5  # 5 cm per grid cell
...
def update_occupancy_map(center_x, center_y, distance, marker_size):
    """
    Updates the occupancy map based on the detected marker's position and size.

    Args:
        center_x (float): X coordinate of the marker's center in image space (pixels).
        center_y (float): Y coordinate of the marker's center in image space (pixels).
        distance (float): Distance of the marker from the camera in meters.
        marker_size (float): Real-world size of the marker in meters.
    """

    # Step 1: Convert image coordinates to real-world coordinates (assuming robot is at the origin)
    # The camera is pointing straight ahead, so we only use distance for Z-axis
    real_x = distance  # The marker is directly in front of the robot in real-world space
    real_y = (center_x - frame_center_x) * (distance / focal_length)  # Convert X to meters

    # Step 2: Convert real-world coordinates to grid indices
    grid_x = int(real_x / grid_resolution)
    grid_y = int(real_y / grid_resolution)

    # Step 3: Calculate the radius of the area occupied by the marker
    radius = int(marker_size / grid_resolution / 2)  # Divide marker size by grid resolution

    # Step 4: Mark the corresponding grid cells as occupied
